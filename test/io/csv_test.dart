...
//    }
    ...
    val expect = Frame(Vec("1", "4", "5", "7"), Vec("25", "55", "9", "8"),
        Vec("36", "6", "38", "9")).setColIndex(Index("a", "b,c,d", "e"));

    expect(frame, equals(expect));
  });

  test("csv int parsing works", () {
    val data = """a,"b,c,d",e
        |1,2,3
        |4,5,"test",
        |7, "8",    "9",   """.stripMargin();
...
//    }
  ...
  test("csv fails on irregular row", () {
    val data = """a,"b,c,d",e
        |1,2,3
        |4,5
        |7, "8",    "9",   """.stripMargin();
...
//    }
    ...
    expect(CsvParser.parse()(src), throwsA(ArrayIndexOutOfBoundsException));
  });

  test("csv parsing still works when final field is empty", () {
    val data = """1,2,3
       |1,2,""".stripMargin();
...
//    }
    ...
    expect(
        CsvParser.parse()(src), isNot(throwsA(ArrayIndexOutOfBoundsException)));
  });
}
