...
indexTest() {
  group("Index Joins", () {
    test("Unique sorted left join", () {
      var ix1 = new Index([0, 1, 2]);
      var ix2 = new Index([1, 2, 3]);

      var res = ix1.join(ix2, JoinType.LeftJoin);

      expect(res.index, equals(new Index([0, 1, 2])));
    ...
    test("Unique sorted right join", () {
      ...
      expect(res.index, equals(new Index([1, 2, 3])));
    ...
    test("Unique sorted inner join", () {
      ...
      expect(res.index, equals(new Index([1, 2])));
    ...
    test("Unique sorted outer join", () {
      ...
      expect(res.index, equals(new Index([0, 1, 2, 3])));
      expect(res.lTake /*.get*/, equals([0, 1, 2, -1]));
      expect(res.rTake /*.get*/, equals([-1, 0, 1, 2]));
    });

    test("Unique unsorted left join", () {
      var ix1 = new Index([1, 0, 2]);
      var ix2 = new Index([2, 3, 1]);

      var res = ix1.join(ix2, JoinType.LeftJoin);

      expect(res.index, equals(new Index([1, 0, 2])));
      expect(res.lTake, isNull);
    ...
    test("Unique unsorted right join", () {
      var ix1 = new Index([1, 0, 2]);
      var ix2 = new Index([2, 1, 3]);

      var res = ix1.join(ix2, JoinType.RightJoin);

      expect(res.index, equals(new Index([2, 1, 3])));
    ...
    test("Unique unsorted outer join", () {
      ...
      expect(res.index, equals(new Index([1, 0, 2, 3])));
      expect(res.lTake /*get*/, equals([0, 1, 2, -1]));
      expect(res.rTake /*get*/, equals([1, -1, 0, 2]));
    });

    test("Non-unique sorted left join", () {
      var ix1 = new Index([0, 1, 1, 2]);
      var ix2 = new Index([1, 2, 2, 3]);

      var res = ix1.join(ix2, JoinType.LeftJoin);

      expect(res.index, equals(new Index([0, 1, 1, 2, 2])));
    ...
    test("Non-unique sorted right join", () {
      ...
      expect(res.index, equals(new Index([1, 1, 2, 2, 3])));
    ...
    test("Non-unique sorted inner join [case 3]", () {
      ...
      expect(res.index, equals(new Index([1, 1, 2, 2, 2, 2])));
    ...
    test("Non-unique sorted outer join", () {
      ...
      expect(res.index, equals(new Index([0, 1, 1, 2, 2, 3])));
    ...
    test("Non-unique unsorted right join", () {
      ...
      expect(res.index, equals(new Index([1, 1, 3, 2, 2])));
    ...
    test("Non-unique unsorted outer join", () {
      ...
      expect(res.index, equals(new Index([1, 1, 2, 2, 0, 3])));
