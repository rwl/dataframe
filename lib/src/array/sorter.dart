...
abstract class Sorter<T> {
  Array<int> argSorted(Array<T> arr);
  Array<T> sorted(Array<T> arr);
}

class Sorter {
  /*private*/ Array<double> nanToNegInf(Array<double> arr) {
    val tmp = arr.clone();
    var i = 0;
    while (i < tmp.length) {
      val ti = tmp(i);
...
//  /*private*/ nanToNegInf(arr: Array[Float]): Array[Float] = {
//    val tmp = arr.clone()
//    var i = 0
//    while (i < tmp.length) {
//      val ti = tmp(i)
...
object boolSorter extends Sorter<bool> {
  def argSorted(arr: Array<bool>) = VecBool.argSort(arr)
  def sorted(arr: Array<bool>) = VecBool.sort(arr)
}
