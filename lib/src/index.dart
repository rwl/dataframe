...
import 'vec.dart';
import 'vec/vec.dart' show VecImpl;
 ...
 */
class Index<T> /*[@spec(Boolean, Int, Long, Double) T] extends Serializable*/ {
  Locator _locator;
...
//  List<int> apply(List<T> keys) => apply(keys);// : _*);
   ...
   */
  Index<T> without(Array<int> locs);
  ...
  Index<C> concat /*[B, C]*/ (Index<
      B> other) /*(implicit p: Promoter[T, B, C], mc: ST[C], oc: ORD[C])*/;
   ...
   */
  Index<T> sliceBy(T from, T to, [bool inclusive = true]) {
    if (inclusive) {
...
//  }
   ...
   */
  Scalar<T> get first => (length > 0) ? at(0) : NA;

  /**
   * Returns the last element of the Index, or NA if there is none
   */
  Scalar<T> get last => (length > 0) ? at(length - 1) : NA;
   ...
   */
  int findOne(bool pred(T arg)) => VecImpl.findOne(toVec)(pred)(scalarTag);
   ...
   */
  Index<V> stack /*[U, V]*/ (
          Index<U> other) /*(implicit Stacker[T, U, V] ev)*/ =>
      ev(this, other);
   ...
   */
  Option<Array<Int>> getIndexer(Index<T> other) {
...
//    }
   ...
   */
  Index<B> map /*[@spec(Boolean, Int, Long, Double) B: ST: ORD]*/ (B f(T arg));
  ...
  int get hashCode => toArray().fold(
      1,
      (a, b) =>
          a * 31 + b.hashCode); //toVec().foldLeft(1)(_ * 31 + _.hashCode());
...
//    val maxf = (List<int> a, List<String> b) => (a zip b).map(v => v._1.max(v._2.length));
...
//      var vlens = util.grab(varr, half).map(sm.strList(_)).foldLeft(sm.strList(varr(0)).map(_.length))(maxf);
...
//    Index((rrule.copy(count = None) withUntil end from start).toSeq : _*)
   ...
   */
  Index<C> empty() /*[C: ST: ORD]*/ => Index(Array.empty[C]);
   ...
   * @tparam C Type of elements in array
   */
  /*implicit*/ arrayToIndex /*[C: ST: ORD]*/ (Array<C> arr) => Index(arr);
   ...
   * @tparam C Type of elements in Vec
   */
  /*implicit*/ vecToIndex /*[C: ST: ORD]*/ (Vec<C> s) => Index(s.toArray);
}
